...
class QueryInterfaceRequestTests: GRDBTestCase {
    ...
    func testSortWithCollation() throws {
        ...
        #if GRDBCUSTOMSQLITE
        XCTAssertEqual(
            sql(dbQueue, tableRequest.order(Col.dataMD5.collating(.nocase).ascNullsLast)),
            "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE ASC NULLS LAST")
        XCTAssertEqual(
            sql(dbQueue, tableRequest.order(Col.dataMD5.collating(.nocase).descNullsFirst)),
    ...
    func testReverseWithCollation() throws {
        ...
        #if GRDBCUSTOMSQLITE
        XCTAssertEqual(
            sql(dbQueue, tableRequest.order(Col.dataMD5.collating(.nocase).ascNullsLast).reversed()),
            "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE DESC NULLS FIRST")
        XCTAssertEqual(
            sql(dbQueue, tableRequest.order(Col.dataMD5.collating(.nocase).descNullsFirst).reversed()),
            "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE ASC NULLS LAST")
        #endif
    }
